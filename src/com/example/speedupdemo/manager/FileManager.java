...
public class FileManager {
	public static final String TYPE_ANY = "全部";

	public static final String TYPE_TXT = "文档";

	public static final String TYPE_VIDEO = "视频";

	public static final String TYPE_AUDIO = "音频";

	public static final String TYPE_IMAGE = "图像";

	public static final String TYPE_ZIP = "压缩包";

	public static final String TYPE_APK = "程序包";
	...
	/**{文件后缀名，文件对应图像名称,　文件所属类型}*/
	public static String[] keys = { TYPE_ANY, TYPE_TXT, TYPE_VIDEO, TYPE_AUDIO,
			TYPE_IMAGE, TYPE_ZIP, TYPE_APK };
	...
	/** 存储信息的集合 */
	public HashMap<String, ArrayList<FileInfo>> infoMap = new HashMap<String, ArrayList<FileInfo>>();

	/** 存储各个总文件大小的集合 */
	public HashMap<String, Long> sizeMap = new HashMap<String, Long>();
	...
	public void initMap() {
		for (int i = 0; i < keys.length; i++) {
			infoMap.put(keys[i], new ArrayList<FileInfo>());
			sizeMap.put(keys[i], 0l);
		}
	}
	...
	public void searchFile(File file) {
		if (file.exists() && file.canRead() && file.isDirectory()) {// 文件存在且能读取，是文件夹
			File[] listFiles = file.listFiles();
			// 加上listFiles！=null,调试结果log文件为null
			for (int i = 0; listFiles != null && i < listFiles.length; i++) {
				if (listFiles[i].isFile()) {
					// 文件的大小
					long size = listFiles[i].length();
					String sizeStr = Formatter.formatFileSize(context, size);
					// 文件的名称
					String name = listFiles[i].getName();
					// 文件的路径
					String path = listFiles[i].getAbsolutePath();
					// 最后修改的时间
					long time = listFiles[i].lastModified();
					...
					if (start > 0) {
						for (int j = 0; j < ICON_TYPE_Table.length; j++) {
							String[] icon_type = ICON_TYPE_Table[j];
							String postfix = name.substring(start + 1);// 获取后缀名
							if (icon_type[0].equals(postfix)) {
								icon = icon_type[1];
								type = icon_type[2];
								break;// 记得break！！
							}
						...
						for (int j = 0; j < MIME_Table.length; j++) {
							String[] mimes = MIME_Table[j];
							String postfix = name.substring(start + 1);
							if (mimes[0].equals(postfix)) {
								mime = mimes[1];//获取打开方式
								break;
							}
						}
					...
					FileInfo info = new FileInfo(icon, name, date, sizeStr,
							size, path, type, mime);
					// 出现空指针异常:初始化集合的时候用了infoMap的长度！要改成keys数组长度
					infoMap.get(TYPE_ANY).add(info);// 将所有信息添加到any键对应的集合
					long sizeAll = sizeMap.get(TYPE_ANY);
					sizeMap.put(TYPE_ANY, sizeAll + size);
					if (infoMap.containsKey(type)) {// 判断是否含有该类型的键
						infoMap.get(type).add(info);
						sizeMap.put(type, sizeMap.get(type) + size);
					}
					listener.onTextChange(type);
				} else {
	...
	public interface OnTextChangeListener {
		void onTextChange(String type);// 不确定是什么类型，通过传参
	}
